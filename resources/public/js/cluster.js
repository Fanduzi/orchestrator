function Cluster() {
    ...
    function preVisualizeInstances() {
        ...
        instances.forEach(function(instance) {
            ...
            KnownRegions.push(instance.Region);
        });
        ...
        for (i = 0; i < knownDCs.length; ++i) {
            ...
            } else if (!isAnonymized() && knownDCs[i]) {
                $('<span>' + knownDCs[i] + '</span>').addClass("dc").appendTo('#cluster_legend');
            }
        }
        KnownRegions = uniq(KnownRegions)
        if (!isAnonymized()) {
            $('<span>Regions:</span>').appendTo('#cluster_legend');
        }
        for (i = 0; i < KnownRegions.length; ++i) {
            if (!isAnonymized() && KnownRegions[i]) {
                $('<span>' + KnownRegions[i] + '</span>').addClass("dc").appendTo('#cluster_legend');
            }
        }
    }
